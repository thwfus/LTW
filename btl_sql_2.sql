...
(1, 'Natural Wood Table Review After Two Weeks of Use', 'The table feels solid, the surface is smooth, and the color looks close to the product photos on the website. It works well for daily use and gives the room a warmer feeling.', 'uploads/reviews/natural-wood-table.jpg', 'approved', 'product', 'Approved sample review.', '2026-05-05 09:00:00', 11, 2, 1),
...
(4, 'Customer Service Before Purchase Was Helpful', 'The staff answered my questions clearly and suggested suitable products based on my room size and budget. This made the purchase decision easier.', 'uploads/reviews/customer-service.jpg', 'pending', 'service', NULL, '2026-04-26 14:20:00', 14, NULL, NULL),
(5, 'Packaging and Delivery Review', 'The product was packed carefully and delivered in good condition. The corners and surface were protected well during shipping.', 'uploads/reviews/packaging-delivery.jpg', 'approved', 'delivery', 'Approved sample review.', '2026-04-22 16:45:00', 15, NULL, 3),
(6, 'Comfortable Wooden Chair for Daily Use', 'The wooden chair is comfortable for daily use and feels stable. The finish is simple but elegant, and it matches the style of my room.', 'uploads/reviews/wooden-chair.jpg', 'hidden', 'product', 'Hidden for admin review in sample data.', '2026-04-18 11:10:00', 16, 1, 4);
...
(1, 'This review is useful. I am also interested in the oak table.', 'approved', 'Approved sample comment.', '2026-05-05 10:00:00', 1, 12, 1),
(2, 'The product photos look clear. Thanks for sharing the real experience.', 'approved', 'Approved sample comment.', '2026-05-05 10:30:00', 1, 13, 1),
...
(5, 'I had a similar customer service experience before ordering.', 'pending', NULL, '2026-04-26 15:00:00', 4, 16, NULL),
...
--
ALTER TABLE `review_cmt`
  MODIFY `review_cmt_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;
...
--
ALTER TABLE `review_post`
  ADD CONSTRAINT `fk_review_post_customer` FOREIGN KEY (`user_id`) REFERENCES `customer` (`user_id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
ALTER TABLE `review_cmt`
  ...
  ADD CONSTRAINT `fk_review_cmt_customer` FOREIGN KEY (`user_id`) REFERENCES `customer` (`user_id`) ON DELETE CASCADE ON UPDATE CASCADE,
